			...
			populateVisible: function() {
					...
					(p.lastpage===undefined||parseInt((tbot + scrollTop + div - 1) / div,10) <= p.lastpage))
				{
					...
					if (tbot >= 0 || npage < 2 || p.scroll === true) {
						page = Math.ceil((tbot + scrollTop) / div) + 1;
						ttop = -1;
					} else {
						ttop = 1;
					}
		...
		updatepager = function(rn, dnd) {
			...
			if (ts.p.scroll) {
				...
				if (rh) {
                    ...
                    // By Devir
					var height2 = height;
					if (ts.p.page == p.lastpage)
					    height2 = $(ts.grid.bDiv).find("table:first").outerHeight();
					$(">div:first", ts.grid.bDiv).css({ height: height2 }).children("div:first").css({ height: top, display: top ? "" : "none" });
					// By Devir  $(">div:first", ts.grid.bDiv).css({ height: height }).children("div:first").css({ height: top, display: top ? "" : "none" });
